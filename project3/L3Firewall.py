...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		self.fwconfig = list()
		self.port_table = dict()    # To store MAC -> IP mapping
	...
	def _handle_PacketIn(self, event):
		...
		# to bind mac with ip
		self.port_security(match.dl_src, match.nw_src)
	...
	def port_security(self, src_mac, src_ip):
		if src_mac is None or src_ip is None:
			return
		if str(src_mac) not in self.port_table: # if F.SrcMAC is new
			# update PT with the mapping F.SrcMAC <--> F.SrcIP;
			self.port_table[str(src_mac)] = str(src_ip)
			log.debug("[port_security] add link between %s => %s", src_mac, src_ip)
		elif self.port_table[str(src_mac)] != str(src_ip):
			# block F.SrcMAC % Block an MAC address that had spoofed multiple IP addresses
			self.add_rule_to_l3config(src_mac = str(src_mac))
			log.debug("[port_security] mac address is reused for another ip: MAC %s existing IP %s new IP %s", src_mac, self.port_table[str(src_mac)], src_ip)
		# else do nothing

	def add_rule_to_l3config(self, src_mac='any', dst_mac='any', src_ip='any', dst_ip='any', nw_proto='tcp'):
		# Set default rule sequence as 1		
		rule_seq = 1
		# Open l3firewall.config file for reading
		with open(l3config, 'r') as csvfile:
			self.rules = csv.DictReader(csvfile)
			for row in self.rules:
				# Check if the rule has already been existed in l3firewall.config file	
				if row['src_mac'] == src_mac and row['dst_mac'] == dst_mac and row['src_ip'] == src_ip and row['dst_ip'] == dst_ip and row['nw_proto'] == nw_proto:
					return
				# increase the rule sequence
				rule_seq += 1
		# Open l3firewall.config file for appending a new rule
                with open(l3config, 'a') as csvfile:
			csvwriter = csv.DictWriter(csvfile, fieldnames=['priority','src_mac','dst_mac','src_ip','dst_ip','src_port','dst_port','nw_proto'])
			log.debug("Saving individual rule parameters in rule dict !")
			csvwriter.writerow({
				'priority': rule_seq,
				'src_mac' : str(src_mac),
				...
				'src_port': 'any',
				'dst_port': 'any',
				'nw_proto': str(nw_proto)
			})
			log.debug("Added a new rule")
